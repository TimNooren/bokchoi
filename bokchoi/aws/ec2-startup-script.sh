...
echo "Downloaded and unpacked project zip" | /tmp/cloudwatch_logger.py init
...
echo "Installed requirements" | /tmp/cloudwatch_logger.py init
...
then
    ...
    echo "Running app" | /tmp/cloudwatch_logger.py init

    python3 -u {entrypoint} | ./cloudwatch_logger.py app
    aws s3 cp /var/log/cloud-init-output.log s3://{bucket}/cloud-init-output.log

    echo "Finished running app" | /tmp/cloudwatch_logger.py end

    if [ "{shutdown}" = "True" ]
    then
        echo "Shutting down..." | /tmp/cloudwatch_logger.py end
